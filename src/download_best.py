...
def download_model(artifact_uri, dest_path):
    mlflow.artifacts.download_artifacts(artifact_uri=artifact_uri, dst_path=dest_path)
...
def main():
    ...
    for model_info in client.search_registered_models(max_results=1000):
        ...
        if any(alias_pattern.match(key) for key in model_aliases.keys()):
            print(model_info)
            print(
                f"Downloading '{model_info.latest_versions[0].name}' model with alias '{model_aliases}'"
            )
